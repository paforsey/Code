...
**Fig 1.1**  The boxplot graph appears to indicate that the supplement in OJ results in higher average tooth lenth than the supplement in VC.  However, the amount of overlap between the two seems to indicate that the difference may not be statistically significant.  The two supplements may on average perform the same.
...
**Fig 1.2**  The boxplot graph appears to indicate that does and tooth length are positively correlated; as the dose increases, tooth length also increases.  The lack of overlap between the three seems to indicate that the difference may be statistically significant.  
```{r Exploratory2, include=T, eval=T, echo=T}
#   The Effect of Dose on Tooth Growth
    g2 <- ggplot(ToothGrowth, aes(x=factor(dose), y=len, fill=factor(dose)))
    g2 <- g2 + geom_boxplot() 
    g2 <- g2 + ggtitle("Fig 1.2: Tooth Growth vs Dose Only") + labs(x="Dose", y="Tooth Length")
    g2
```
  
**Fig 1.3**  The boxplot graph appears to indicate that does and tooth length are positively correlated for both supplements; as the dose increases, tooth length also increases.  It is interesting to note that OJ results in higher levels of tooth length for the 0.5mg and 1.0mg doses.  However, there appears to be little or no difference for the 2.0mg dose.  
...
#   The Combined Effects of Supplement & Dose on Tooth Growth
    ...
    g3 <- g3 + ggtitle("Fig 1.3: Tooth Growth vs Dose by Supplement") + labs(x="Dose", 
        y="Tooth Length")
    g3
```
...
Question #3: Does the supplement given the same dose result in different levels of tooth growth?  
