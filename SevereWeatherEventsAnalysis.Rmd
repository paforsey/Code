...
#   Geograhic Representation of Property Damage by State
    ...
    plot(Geo1);
```
Figure 1.1: Texas was the hardest hit with a total of $19.5 Billion in combined property and crop damage from JAN 1950 through NOV 2011.
...
#   Top 5 EVTYPE
    ...
    g1 <- g1 + labs(x="Texas", y="Property Damage (%)")
    g1
```

Figure 1.2: The top five severe weather events responsible well over 80% of the total amount of property in the state of Texas: 1) Hail, 2) Flood, 3) Flash Flood, 4) Wild Fire, and 5) TSTM Wind.
...
Figure 2.1: Again, Texas was the hardest hit with a total of 19,033 in the number of injuries and fatalities from JAN 1950 through NOV 2011.
...
Figure 2.2: The top five severe weather events attributed to well over 80% of the total number fatalities and injuries occuring in the state of Texas: 1) Tornado, 2) Flood, 3) Flash Flood, 4) Heat, and 5) TSTM Wind.


Note:
The scope of this analysis does not include US territories, such as the Virgin Islands or Guam, as severe weather events in these areas represent less than 2% of the overall the property dammage, injury, and fatality statistics contained in the NOAA Storm Events Database.
```{r echo=F}
    options(op);
```
