...
#   Top 10 EVTYPE
    ECONOMIC <- (ECONOMIC[order(ECONOMIC$STATE_NAME, -ECONOMIC$ECONOMIC), ]);    
    HEALTH <- head(HEALTH[order(-HEALTH$HEALTH), ], n=5);
...
```{r EconomicsImpact, include=T, echo=T, results='asis', tidy=F}
    ...
    g <- g + labs(x="State(s)", y="Injuries/Fatalities ($ Millions)")
    g
...
```{r PopulationHealthGraph, echo=F, results='asis', tidy=F}
